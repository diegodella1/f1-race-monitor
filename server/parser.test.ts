...
test('lap packet parses distance and keeps the last reliable impossible interval',()=>{
  ...
  buffer.writeUInt8(1,base+37);
  const result=parsePacket(buffer,state)!;
  assert.equal(result.player.vehicleIndex,1);assert.equal(result.drivers[0].lapInvalid,true);assert.equal(result.drivers[1].lapInvalid,false);
...
test('participant refresh preserves the lap quality needed for weather comparisons',()=>{
  const state=initialState();state.sessionUid='123';state.drivers=[{vehicleIndex:0,position:1,name:'LEADER',team:'Ferrari',lap:2,sector:1,gap:'LEADER',interval:'—',tyre:'HARD',tyreAge:2,pit:false,lapInvalid:false,lastLap:'1:30.000'}];
  const buffer=packet(4,1470);buffer.writeUInt8(1,29);buffer.write('LEADER',40,'ascii');
  const result=parsePacket(buffer,state)!;
  assert.equal(result.drivers[0].lapInvalid,false);assert.equal(result.drivers[0].lastLap,'1:30.000');
});
