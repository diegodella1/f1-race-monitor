...
const app=express(),http=createServer(app),io=new Server(http,{cors:{origin:true}});app.use(cors());app.use(express.json());
let settings:Settings={udpPort:Number(process.env.UDP_PORT)||20777,demoMode:process.env.DEMO_MODE==='true',autoSave:true,demoSession:'RACE'}; let state=initialState(); let udp:dgram.Socket|null=null; let demoTimer:NodeJS.Timeout|null=null; let tick=0; let disconnectTimer:NodeJS.Timeout|null=null;
type PacketDiagnostic={id:number;size:number;count:number;firstSeen:number;lastSeen:number;sample:string};
const diagnostics={startedAt:Date.now(),source:null as string|null,validPackets:0,invalidPackets:0,packets:{} as Record<string,PacketDiagnostic>};let lastDiagnosticWrite=0;
const store=new SessionStore(path.resolve('data/f1-monitor.sqlite')); settings=store.loadSettings(settings); const ips=()=>Object.values(os.networkInterfaces()).flat().filter((x):x is os.NetworkInterfaceInfo=>!!x&&x.family==='IPv4'&&!x.internal&&!x.address.startsWith('169.254.')).map(x=>x.address); const primaryIp=()=>ips().find(ip=>ip.startsWith('192.168.')||ip.startsWith('10.')||/^172\.(1[6-9]|2\d|3[01])\./.test(ip))||ips()[0]||'localhost';
...
function publish(){state={...state,telemetry:telemetry.quality(state)};state=strategy.analyze(state);state=coach.analyze(state);state=engineer.analyze(state);for(const stream of streams.values())stream.update(state);streamDirty=true;if(settings.autoSave&&state.status!=='WAITING')store.save(state);}
